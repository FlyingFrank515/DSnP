...
DBAppendCmd::exec(const string& option)
{
   ...
   // check option
   if(!dbjson){
      //use cerr to print error message
      cerr << "Error: DB is not created yet!!" << endl;
      return CMD_EXEC_ERROR;
   }
   vector<string> tokens;
   if(CmdExec::lexOptions(option, tokens, 2)){
      //with (true) two tokens
      int num;
      if(isValidVarName(tokens[0]) && myStr2Int(tokens[1],num)){
         DBJsonElem temp(tokens[0],num);
         
         if(dbjson.add(temp))
            return CMD_EXEC_DONE;
         else{
            cerr << "Error: Element with key \"" << tokens[0] ;
            cerr << "\" already exists!!" << endl;
            return CMD_EXEC_ERROR;
         }
      }
      else if(!isValidVarName(tokens[0])){
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[0]);
      }
      else if(!myStr2Int(tokens[1], num)){
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[1]);
      }
   }
   //for the lexOptions,it will print error message in its
   //so we do not have to print again
...
{  
   ...
   string token;
   if(!CmdExec::lexSingleOption(option, token, true)) return CMD_EXEC_ERROR;
   else{
      //key is not specified->print all the element
      if(token.size() == 0){
         cout << dbjson;
         cout << "Total JSON elements: " << dbjson.size() << endl;
      }
      //key is specified->print JSON element with the the “key”(sensitive)
      else{
         bool find_key = false;
         for(int i = 0 ; i < (int)dbjson.size() ; i++){
         ...
         if(!find_key){
            cerr << "Error: No JSON element with key \""
            << token << "\" is found." << endl;
            return CMD_EXEC_ERROR;
         }
      }
