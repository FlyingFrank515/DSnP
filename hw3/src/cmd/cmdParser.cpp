...
CmdParser::openDofile(const string& dof)
{
   // TODO...
   //point to dofile
   //0 represent NULL(origin)
   //open file->point to the object
   //store now dofile then call new dofile
   //maybe use _dofileStack??
   const int DEPTH = 1024;
   _dofile = new ifstream(dof.c_str());
   if((int)_dofileStack.size() > DEPTH) return false; 
   if(!_dofile->is_open()){
...
CmdParser::closeDofile()
{
   //close now doflie
   //delete it 
   //update the dofile to the original place 
   assert(_dofile != 0);
   // TODO...
   _dofile->close();
   delete _dofile;
   if((int)_dofileStack.size() > 0){
      _dofileStack.pop();
      //if there still are things in dofilestack
      if((int)_dofileStack.size() > 0)
         _dofile = _dofileStack.top();
      //dofilestack is blank then dofile->null
      else _dofile = 0;
   }
   else _dofile = 0;
   return;
...
{
   // TODO...
   //the function is anounced as "const"
   //use const_iterator
   CmdMap::const_iterator itr = _cmdMap.begin();
   while(itr != _cmdMap.end()){
      itr->second->help();
      itr++;
   }
   cout << endl;
...
CmdParser::parseCmd(string& option)
{
   ...
   size_t tok_op_start;
   string tok_1;
   tok_op_start = myStrGetTok(str, tok_1);
   CmdExec* e = getCmd(tok_1);
   if(e == 0){
      cerr << "Illegal command!! (" << tok_1 << ")" << endl;
      return NULL;
   }
   else{
      if(tok_op_start < str.size() - 1){
         for(unsigned int i = tok_op_start + 1 ; i < str.size() ; i++){
            option += str[i];
         }
      }
      return e;
...
CmdParser::getCmd(string cmd)
{
   CmdExec* e = 0;
   // TODO...
   //not const function->use iterator
   string must, opt;
   CmdMap::iterator itr;
   for(itr = _cmdMap.begin() ; itr != _cmdMap.end() ; itr++){
      must = itr->first;
      opt = itr->second->getOptCmd();
      string total = must + opt;
      //use myStrNCmp:check length if s1 >= n
      if(!myStrNCmp(total, cmd, must.size())){
         e = itr->second;
         return e;
      }
   }
...
//used to print the error message and return CmdExecStatus
CmdExecStatus
