...
#define toSizeT(t)      t + ((SIZE_T - (t % SIZE_T)) % SIZE_T)  // TODO
//
...
class MemRecycleList
{
   ...
   T* popFront() {
      // TODO
      // first point to next recycled data
   ...
   void reset() {
      // TODO
      // do not need to return the memory back to system
      // because the memory is in memory block 
      // recyclelist is only used to connect the memory
      if(_nextList != NULL) delete _nextList;
      _first = NULL;
      _nextList = NULL;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
      MemBlock<T> *block_now = _activeBlock->getNextBlock();
      MemBlock<T> *block_del;
      while(block_now != NULL){
         block_del = block_now;
         block_now = block_now->getNextBlock();
         delete block_del;
      }
      // use "memblock" 's member function 
      for(int i = 0 ; i < R_SIZE ; i++){
      ...
      // b == _blockSize:dont have to change the _activeblock
      else{
         delete _activeBlock;
         b = toSizeT(b);
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* iter = &(_recycleList[m]);
      bool end = false;
      while(iter->getArrSize() != n){
         if(iter->getNextList() == NULL){
            end = true;
            break;
         }
         iter = iter->getNextList();
      }
      //if didnt find 
      if(end){
         MemRecycleList<T>* temp =  new MemRecycleList<T>(n);
         iter->setNextList(temp);
         iter = iter->getNextList();
      }
      return iter;
   }
   ...
   T* getMem(size_t t) {
      ...
      // TODO
      // popfront:return _first or NULL
      MemRecycleList<T> *temp = getMemRecycleList(n);
      ret = temp->popFront();
      if(ret != NULL){
         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
   ...
   size_t getNumBlocks() const {
      // TODO
      size_t num = 0;
      MemBlock<T>* block_now = _activeBlock;
      while(block_now != NULL){
         num += 1;
         block_now = block_now->getNextBlock();
      }
